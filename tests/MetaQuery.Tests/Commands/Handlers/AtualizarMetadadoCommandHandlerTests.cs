...
{
    public class AtualizarMetadadoCommandHandlerTests
    {
        ...
        private readonly Mock<ILogger<AtualizarMetadadoCommandHandler>> _loggerMock;
        private readonly AtualizarMetadadoCommandHandler _handler;

        public AtualizarMetadadoCommandHandlerTests()
        {
            _repositoryMock = new Mock<IMetadadosRepository>();
            _unitOfWorkMock = new Mock<IUnitOfWork>();
            _notificationContextMock = new Mock<INotificationContext>();
            _loggerMock = new Mock<ILogger<AtualizarMetadadoCommandHandler>>();

            _handler = new AtualizarMetadadoCommandHandler(
        ...
        public async Task Handle_ComDadosValidos_DeveChamarBeginTransaction()
        {
            ...
            var command = new AtualizarMetadadoCommand
            {
                Id = 1,
        ...
        [Fact]
        public async Task Handle_ComErroNoAtualizar_DeveChamarRollback()
        {
            // Arrange
            var metadado = TabelaDinamica.Criar("CLIENTES", "ID,NOME", "ID", visivelParaIA: true);
            var command = new AtualizarMetadadoCommand
